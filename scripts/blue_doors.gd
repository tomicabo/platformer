...
func _on_Area2D_body_enter( body ):
	if(body extends preload("res://scripts/player.gd") && get_node("/root/globals").getPlayerDead() != true):
		if(get_node("/root/globals").getBlueKeyState() == true):
			get_node("AnimationPlayer").play("open")
			if get_node("/root/globals").getSound() != false:
				get_node("SamplePlayer").play("doors_open")
			get_node("/root/globals").setBlueKeyFalse()